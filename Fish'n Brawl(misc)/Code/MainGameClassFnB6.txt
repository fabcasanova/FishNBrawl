...
import pack.phatsmalone.FnB.input.Keyboard;
...
public class MainGame extends Canvas implements Runnable {
	...
	private static final long serialVersionUID = 1L;								//1 is casted as a long data type
				...
				public MainGame() {
					...
					key = new Keyboard();														// instance that takes key inputs
					addKeyListener(key);
				...
				public static void main(String args[]) {
					...
					game.requestFocusInWindow(false);
				...
				public void update(){
					...
					key.update();																			//updates events and key input calculations
					if (key.up) y--;
					if(key.down) y++;
					if(key.left) x--;
					if(key.right) x++;
				...
				public void render() {
					...
					screen.render(x,y);									//renders each pixel to the screen
